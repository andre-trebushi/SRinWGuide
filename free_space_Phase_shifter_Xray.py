...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov  4 11:44:48 2022
...
# drift_b = Undulator(Kx=K*0.0000001, nperiods=0.1, lperiod=0.1, eid="drift_b", phase=-np.pi/2, end_poles=0)
# drift_a = Undulator(Kx=K*0.0000001, nperiods=0.1, lperiod=0.1, eid="drift_a", phase=-np.pi/2, end_poles=0)
...
p_array.tau()[:] = 0
U, E = track4rad_beam(p_array, lat_green, energy_loss=False, quantum_diff=False, accuracy=1)#
...
omega = np.linspace(omega_r*(1-0.9999), omega_r*(1+3), 501)
...
f_5 = Green_func_integrand(x0, y0, z0, x, y, z, omega, ux, uy, uz, order=3, gradient_term=True, Green_func_type='free_space')
...
f = Green_func_integrand(x0, y0, z0, x, y, z, omega, ux, uy, uz, order=3, gradient_term=True, Green_func_type='free_space')#, **pipe)
...
# fig.suptitle("{} THz, {} eV".format(round(THz), round(THz*1e12*h_eV_s, 2)) + r'$, z_0$ = ' + str(z0) + ' m', fontsize=16)
