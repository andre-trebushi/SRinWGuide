...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 11 13:56:59 2022
...
und = Undulator(Kx=K, nperiods=nperiods, lperiod=lperiod, eid="und", phase=0, end_poles=0)
...
omega = np.linspace(omega_r*(1-0.9999), omega_r*(1+3), 1000)
...
#%%
f = Green_func_integrand(x0, y0, z0, x, y, z, omega, ux, uy, u_z, order=5, gradient_term=True, Green_func_type='free_space')
E_x, E_y = Green_func_integrator(f, z, order=5)
...
f = Green_func_integrand(x0, y0, z0, x, y, z, omega, ux, uy, u_z, order=3, gradient_term=True, Green_func_type='free_space')#, **pipe)
E_x, E_y = Green_func_integrator(f, z, order=2) 
...
ax1_twin = ax1.twinx() 
ax1_twin.plot(theta_y, (Ix[n//2, :, 0] - I_oclot[0, n//2, :])/np.max(Ix, I_oclot) *1e2, label=r'Norm. discrepancy', linewidth=0.75, color='red', alpha=1)
