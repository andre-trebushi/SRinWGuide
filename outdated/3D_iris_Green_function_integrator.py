...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 25 16:29:26 2022
...
def G(x0, y0, z0, x, y, z, omega, n_list=[0], k_list=[1]):
    ...
    for nk in itertools.product(n_list, k_list):
        ...
        G_nk_array.append(G_nk)
        # print('Green function shape = ', np.shape(Green))
        
    return np.array(Green), np.array(G_nk_array)
...
def nubla_G(Green, x0, y0, z0, x, y, z, h='machine', n_list=[0], k_list=[1]):
    ...
    dG_x = G(x0, y0, z0, x + dx, y, z, omega, n_list=n_list, k_list=k_list)[0] - Green
    dG_y = G(x0, y0, z0, x, y + dy, z, omega, n_list=n_list, k_list=k_list)[0] - Green
...
omega = np.linspace(omega_r*(1-0.999), omega_r*(1+12), 300)#np.linspace(2*np.pi*(E-200)/h_eV_s , 2*np.pi*(E+6000)/h_eV_s, 2000)
...
k_0 = 10
k_list=np.arange(k_0, k_0 + 30, 10)
# n_list = [0]
# k_list = [1]
Green_nk = G(x0, y0, z0, x, y, z, omega, n_list=n_list, k_list=k_list)[1]

fig, ax1 = plt.subplots(figsize=(10, 5))

for k, k_iter in zip(k_list, range(len(k_list))):
    Green = Green_nk[k_iter]
    
    nub_G_x, nub_G_y = nubla_G(Green, x0, y0, z0, x, y, z, n_list=n_list, k_list=[k])
    ...
    ax1.plot(omega/1e12/2/np.pi, I, linewidth=1, label='k = {}'.format(k))
...
ax2 = ax1.twinx() 
ax2.plot(X*1e-12/h_eV_s, data, linewidth=1, color='black', linestyle='-', label='OCELOT')
